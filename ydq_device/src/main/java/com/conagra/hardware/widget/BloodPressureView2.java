...
public class BloodPressureView2 extends View {
    ...
    public BloodPressureView2(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public BloodPressureView2(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            int num = (int) (((width/margin) - 2) / 3);
            paint.setColor(getResources().getColor(R.color.quickening_text1));
            int nu1 = (int) (x- (width/marginX - 1))/3 - startTimePosition;
//            LogMessage.doLogMessage("BloodPressureView","nu1 : " + nu1);
            String time1 = TimeUtils.getDate(nu1);
            PointF sF = new PointF();
            sF.x = x1;

            if(mSingleTdMap.containsKey(time1) && x > 1) {
                ...
                if (!TextUtils.isEmpty(mm)) {
                    ...
                    sF.y = 16 * marginY + startY - (fl*marginY)/15;
//
...
//                        sF.y = Float.parseFloat(value);
            ...
            if(x % 3 == 2){
                ...
                cF.x = x1;
                int nu = (int) (x- (width/marginX - 1))/3 - startTimePosition;
                String time = TimeUtils.getDate(nu);
...
//                        cF.y = Float.parseFloat(value);
                ...
                paint.setStrokeWidth(DensityUtil.dp2px(context,2) * 3/4);
            }
...
//            paint.setColor(getResources().getColor(R.color.colorMain));
        ...
        for(int i = 0; i < mSinglePointFList.size(); i++){
            ...
            paint.setStrokeWidth(DensityUtil.dp2px(context,1));
            canvas.drawCircle(p.x,p.y,DensityUtil.dp2px(context,2),paint);
    ...
    public void updataData(BloodPressureListModel model, String currentTime){
//        FetalMovementListBakeModel dataWeekBeanList = fetalMovementList.get(position);
        ...
        if(null != model.getRows()) {
            mCountTdMap.clear();
            mSingleTdMap.clear();
            mSinglePointFList.clear();
            mCountPointFList.clear();
            for (BloodPressureRowBean dataWeekBean : model.getRows()) {
                if (dataWeekBean != null) {
//                    LogMessage.doLogMessage("BaseActivity"," fragment : " + dataWeekBean.getRecordDate());
                    String time = TimeUtils.getTime(dataWeekBean.getCreateTime(),"yyyy-MM-dd HH:mm:ss","yyyy-MM-dd");
                    int day = TimeUtils.intervalDay(TimeUtils.getYMDData(0),time);
                    time = TimeUtils.getDate(-day);
                    mCountTdMap.put(time,((int)dataWeekBean.getSystolicPressure()) + "");
                    addSingleValue(time, TypeApi.TIME_IVTERVAL_AM, ((int)dataWeekBean.getDiastolicPressure()) + "");
                }
            }
