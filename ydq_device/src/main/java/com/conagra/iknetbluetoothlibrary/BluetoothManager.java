...
import com.conagra.iknetbluetoothlibrary.util.FrameUtil;
import com.conagra.iknetbluetoothlibrary.util.PermissionUtil;
	...
	private class MyReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothService.ACTION_BLUETOOTH_CONNECT.equals(action)) {
				...
				if(mOnBTMeasureListener !=null&&deviceList.size()>0){
					mOnBTMeasureListener.onConnected(connect, deviceList.get(0));
				}
...
//			SharedPreferencesUtils.addBloodPressure(addr);
//
//		}else if(!SharedPreferencesUtils.isExistAboutBloodPressure(addr)){
//			Toast.makeText(mContext,"该设备无效，请联系医生添加设备",Toast.LENGTH_SHORT).show();
//			return;
//		}
		if(FrameUtil.isServiceRunning("com.conagra.iknetbluetoothlibrary.BluetoothService", mContext)){
