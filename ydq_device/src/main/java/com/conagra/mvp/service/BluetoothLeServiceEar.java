 ...
 */
@SuppressLint("NewApi")
public class BluetoothLeServiceEar extends Service {
	private final static String TAG = BluetoothLeServiceEar.class.getSimpleName();
	...
	private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
		...
		if (data != null && data.length > 0) {
			...
			Log.e(TAG, "DATA - " + stringBuilder.toString());
			stringBuilder = null;
		}
		...
		if (UUID_SLAVE2HOST.equals(characteristic.getUuid())) {
			...
			if (arrayOfByte != null && arrayOfByte.length == dataLen) {
				...
				//判断是否发送结果命令
				if (command == sendResultCommand && (int) arrayOfByte[data1Index] == data1) {
					//计算校验和
					...
					if ((byte) dataSum == arrayOfByte[arrayOfByte.length - 1]) {
						...
						localIntent.putExtra("Ear", String.format("%d", (int) arrayOfByte[resultIndex]));
	...
	public class LocalBinder extends Binder {
		BluetoothLeServiceEar getService() {
			return BluetoothLeServiceEar.this;
		}
	}
...
//		Log.d(TAG, "count=" + this.count);
		...
		byte[] arrayOfByte = { -113, 106, 114, 90, 85, -86, -69, 0 };
		arrayOfByte[7] = ((byte) (0xFF & arrayOfByte[0] + arrayOfByte[1]
				+ arrayOfByte[2] + arrayOfByte[3] + arrayOfByte[4]
				+ arrayOfByte[5] + arrayOfByte[6]));
		Log.i(TAG, "this.h2s = null ? " + (this.h2s == null));
		this.h2s.setValue(arrayOfByte);
		writeCharacteristic(this.h2s);
		this.count = (1 + this.count);
		Log.d(TAG, "count=" + this.count);
